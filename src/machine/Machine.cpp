...
void Machine::InitPageDirectory()
{
	 ...
	 */
	PageDirectory* pPageDirectory = (PageDirectory*)(PROCESS0_PAGE_DIRECTORY_BASE_ADDRESS + KERNEL_SPACE_START_ADDRESS);
	
	/* 填写页目录（0x202#页表）的第0项，使线性地址0-4M映射到物理内存0-4M */
	...
	pPageDirectory->m_Entrys[0].m_PageTableBaseAddress = USER_ZERO_PAGE_TABLE_BASE_ADDRESS >> 12;
	...
	pPageTable->m_Entrys[PageTable::ENTRY_CNT_PER_PAGETABLE-1].m_PageBaseAddress=0x400;	//0#进程PPDA
...
//	tss.m_CR3 = 0x200000;	/* Physical base address of page directory */
	tss.m_CR3=Machine::PROCESS0_PAGE_DIRECTORY_BASE_ADDRESS;
