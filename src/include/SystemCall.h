...
public:
	...
	static int copy_to_user(void* user_dst, const void* kernel_src, int len);
...
private:
	...
	/*	51 = getproc	count = 4	*/
	static int Sys_Getproc();//需要4个参数：代码段起始物理地址x_caddr、代码段长度x_size、可交换部分起始物理地址p_addr、可交换部分长度p_size
private:
	/*系统调用入口表的声明*/
