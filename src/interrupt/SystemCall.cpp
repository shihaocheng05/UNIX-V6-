...
/*	50 = getpids	count = 2	*/
// 内核→用户态安全拷贝函数
int SystemCall::copy_to_user(void* user_dst, const void* kernel_src, int len) {
    // 校验指针非空
    if (user_dst == NULL || kernel_src == NULL) {
        return -1;
    }
    ...
    // 直接内存拷贝（在保护模式下，访问非法用户态地址会触发页错误）
    volatile char* dst = (volatile char*)user_dst;
...
int SystemCall::Sys_Getpids() {
    ...
    // 提取用户态传递的两个指针
    unsigned int* user_pid = (unsigned int*)u.u_arg[0];
    unsigned int* user_ppid = (unsigned int*)u.u_arg[1];

    // 校验指针非空
    if (user_pid == NULL || user_ppid == NULL) {
        u.u_error = User::EINVAL;  // 设置错误码
        return 0;
    }

    // 内核数据
    unsigned int kernel_pid = u.u_procp->p_pid;
    unsigned int kernel_ppid = u.u_procp->p_ppid;

    // 安全拷贝到用户态内存
    if (copy_to_user(user_pid, &kernel_pid, sizeof(unsigned int)) != 0) {
        u.u_error = User::EFAULT;  // 拷贝失败，设置错误码
        return 0;
    }
    if (copy_to_user(user_ppid, &kernel_ppid, sizeof(unsigned int)) != 0) {
...
int SystemCall::Sys_Getproc() {
    User& u = Kernel::Instance().GetUser();
    // 1. 提取用户态传递的4个指针参数（按顺序：x_caddr、x_size、p_addr、p_size）
    unsigned long* user_x_caddr = (unsigned long*)u.u_arg[0];
    unsigned int*  user_x_size  = (unsigned int*)u.u_arg[1];
    unsigned long* user_p_addr  = (unsigned long*)u.u_arg[2];
    unsigned int*  user_p_size  = (unsigned int*)u.u_arg[3];
    // 2. 校验所有用户指针非空（避免空指针拷贝）
    if (user_x_caddr == NULL || user_x_size == NULL ||
        user_p_addr == NULL || user_p_size == NULL) {
        u.u_error = User::EINVAL;  // 设置错误码
        return 0;
    }
    // 3. 校验内核态指针非空（避免内核自身空指针解引用崩溃）
    if (u.u_procp == NULL || u.u_procp->p_textp == NULL) {
        u.u_error = User::EFAULT;  // 内核进程控制块/文本段指针无效
        return 0;
    }
    // 4. 提取内核态数据（进程相关信息）
    ...
    if (copy_to_user(user_p_addr, &kernel_p_addr, sizeof(unsigned long)) != 0) {
        u.u_error = User::EFAULT;
        return 0;
    }
    if (copy_to_user(user_p_size, &kernel_p_size, sizeof(unsigned int)) != 0) {
        u.u_error = User::EFAULT;
        return 0;
    }
    u.u_ar0[User::EAX] = 0;
