...
//下面两个函数如果返回值为0说明系统调用正常执行
//该文件中存放钩子函数
int getpids(unsigned int*pid1,unsigned int*pid2)
{
	int res;
	__asm__ volatile ( "int $0x80":"=a"(res):"a"(50),"b"(pid1),"c"(pid2));
	if ( res >= 0 )
		return res;
	return -1;
}

int getproc(unsigned long*x_caddr, unsigned int*x_size, unsigned long*p_addr, unsigned int*p_size)
{
    int res;
    __asm__ volatile (
        "int $0x80"
        :"=a"(res)                  // 输出：%eax = 返回值
        :"a"(51),                   // 输入1：%eax = 系统调用号51
         "b"(x_caddr),              // 输入2：%ebx = 第1参数（x_caddr指针）
         "c"(x_size),               // 输入3：%ecx = 第2参数（x_size指针）
         "d"(p_addr),               // 输入4：%edx = 第3参数（p_addr指针）
         "S"(p_size)
        : "memory"
    );
    if (res >= 0)
