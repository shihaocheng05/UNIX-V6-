...
D:\\UNIX V6++test\\oos\\src\\include\\Allocator.h

增设数据结构PageAllocator，拥有全局静态对象m\_Instance，自主实现页式Alloc和Free方法供PageManager调用分配或释放物理内存，自主实现hasEnoughSpace供PageManager判断是否有足够空间

D:\\UNIX V6++test\\oos\\src\\mm\\Allocator.cpp

增加上述三个函数的实现

D:\\UNIX V6++test\\oos\\src\\machine\\Machine.cpp

对于void Machine::InitPageDirectory()，修改了pPageDirectory的地址；同时修改了0号表项的基地址，让核心页表最后一项映射到0x400物理页框。修改了void Machine::InitUserPageTable()，只初始化0#用户页框。

D:\\UNIX V6++test\\oos\\src\\kernel\\main.cpp

在next里面增加了对Page的初始化。对pProcZero->p\_pgTable的初始化。在最后if分支的1#进程分支不再写用户页表，而是SwtchUStruct。

D:\\UNIX V6++V4\\oos\\src\\include\\MemoryDescriptor.h

修改	PageDirectory\* Initialize()、void EstablishUserPageTable的声明。增加void CopyUserPageTable(PageTable\* pgTable,unsigned int Page\[]);的声明。注释MapToPageTable

D:\\UNIX V6++V4\\oos\\src\\proc\\MemoryDescriptor.cpp

修改Initialize实现与Release()实现。增加void MemoryDescriptor::EstablishUserPageTable、void MemoryDescriptor::CopyUserPageTable(PageTable\* pgTable,unsigned int Page\[])、实现。注释void MemoryDescriptor::MapToPageTable()。

D:\\UNIX V6++V4\\oos\\src\\include\\PageManager.h

重写UserPageManager和KernelPageManager的一些常量，重写AllocMemory和FreeMemory的实现，增加EnoughSpace判断将分配空间是否充足。

D:\\UNIX V6++V4\\oos\\src\\include\\Process.h

增加成员p\_pgTable用于存储页目录虚地址。注释Expand

D:\\UNIX V6++test\\oos\\src\\proc\\Process.cpp

重写Exit，SBreak、SStack

D:\\UNIX V6++test\\oos\\src\\include\\ProcessManager.h

重新定义宏SwtchUStruct(p)，相应修改D:\\UNIX V6++test\\oos\\src\\include\\Assembly.h中的宏FlushPageDirectory()

D:\\UNIX V6++test\\oos\\src\\proc\\ProcessManager.cpp

修改SetupProcessZero、NewProc、Swtch、Sched、Wait、Exec、XSwap

D:\\UNIX V6++V4\\oos\\src\\interrupt\\Exception.cpp

修改PageFault

D:\\UNIX V6++test\\oos\\src\\include\\Text.h

注释XccDec，增加x\_pgTable

D:\\UNIX V6++test\\oos\\src\\proc\\Text.cpp

修改XFree，注释XccDec

D:\\UNIX V6++test\\oos\\src\\include\\Utility.h

在该文件和cpp文件中同步注释了CopySeg2.

D:\\UNIX V6++test\\oos\\src\\kernel\\Utility.cpp

兼容FlushPageDirectory的修改

D:\\UNIX V6++test\\oos\\src\\pe\\PEParser.cpp

修改了PEParser::Relocate(Inode\* p\_inode, int sharedText)，以及PEParser::HeaderLoad(Inode\* p\_inode)，只给sectionHeaders分配了一页用于兼容现在AllocMemory的语义，也是考虑到ImageSectionHeader应该也用不了那么多空间？
...
D:\\UNIX V6++test\\oos\\src\\mm\\SwapperManager.cpp，类似page数组的逻辑，在SwapperManager的构造函数中初始化SwapperPage数组。与pages数组同样的，也设置了空闲物理页链表。AllocSwap/FreeSwap和AllocMemory/FreeMemory的区别在于盘交换区上不用管理物理页框的引用次数（当前系统只会换出RW页到盘交换区）。注意每次将物理页从内存读到磁盘块要释放内存中的物理地址，将磁盘块读到内存同理。
...
D:\\UNIX V6++test\\oos\\src\\include\\PEParser.h，对PEParser的成员采取了更细粒度的划分方式，便于与Exec对接。但this->RDataSize=this->sectionHeaders\[RDATA\_SECTION\_IDX].Misc.VirtualSize和BBS的长度使用的是联合体的成员，可能是一个隐患。释放原进程的栈。PE文件中只存储了堆和栈的大小，没有存储位置。所以还是默认堆在BSS下面，栈紧贴着用户空间底部8M。释放原进程图像物理页时，也考虑到了要按照vm\_area释放而非MemoryDescriptor释放。删除Relocate函数，末尾的kpm.FreeMemory直接移到Exec尾部
